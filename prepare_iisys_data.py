...
"""
Adapted from facebook wav2letter script to prepare iisys data instead 
of librispeech
...
def write_sample(line, idx, dst):
    ...
    os.system(
        "ln -s {src} {dst}".format(
            src=os.path.abspath(os.path.join(os.path.dirname(filename), input + ".wav")),
            dst=basepath + ".wav",
        )
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dst", help="destination directory", default="./iisys")
    
    args = parser.parse_args()

    assert os.path.isdir(
        str(args.src)
    ), "iisys src directory not found - '{d}'".format(d=args.src)
    ...
    for subpath in subpaths:
        ...
        dst = os.path.join(args.dst, "iisys")
