...
int main(int argc, char** argv) {
  ...
  auto runDecoder = [&](int tid, int start, int end) {
    try {
      ...
      evalLogFile.open("decode_log.txt");
      evalLogFile << "elapsed_dec" <<  ",";
      evalLogFile << "sample_id" << "\n";
      ...
      for (int s = start; s < end; s++) {
        ...
        auto finish_dec = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> elapsed_dec = finish_dec - start_inf;
        std::cout << "Elapsed dec time: " << elapsed_dec.count() << " s\n";
        std::cout << "Sample: " << sampleId << std::endl; 
        evalLogFile << elapsed_dec.count() <<  ",";
